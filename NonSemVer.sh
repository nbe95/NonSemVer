...
usage() {
    echo "Usage: $(basename "$0") [-h|--help] [-v|--verbose] [-i|--integer] [--bump-minor|--bump-bugfix] [--test ID] [VERSION]" 1>&2;
    exit 0;
}
...
while true; do
  case "$1" in
    ...
    --test ) TEST_VERSION="$(printf "%04d" "${2:0:4}")"; shift; shift;;
    * ) break;;
  esac
done
...
# Shift places in case of test version
if [ "$TEST_VERSION" -ne 0 ]; then
    build="$(printf "%02d" "$bugfix")$(printf "%02d" "$((10#$TEST_VERSION % 100))")"
    bugfix="$(printf "%02d" "${TEST_VERSION:0:2}")"
fi

# Output parsed version
printf -v output "%02d.%02d.%02d%02d" "$prefix" "$major" "$minor" "$bugfix"
if [ -n "$build" ]; then
